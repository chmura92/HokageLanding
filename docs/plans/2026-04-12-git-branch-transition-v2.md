# Git Branch Transition v2 Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rewrite `SectionTransition.tsx` to render a proper git graph — 3 horizontal lanes with 45° diagonal connectors at branch/merge points, commits placed at real events, a single left→right sweep animation, and bolder lines.

**Architecture:** Full rewrite of the Canvas 2D component. Data is expressed as a flat list of `LineSegment` objects (each has two endpoints in normalised coordinates) plus a flat list of `Commit` positions. A single sweep progress value (0→1 over 1400ms) drives the reveal: each segment draws up to `sweepX`, each commit pops when `sweepX` passes its x position. Bezier/branch/segment structs are all removed. The gradient background and DPR/resize/IO/RAF patterns from the existing file are preserved.

**Tech Stack:** Canvas 2D, `requestAnimationFrame`, `IntersectionObserver`, `ResizeObserver`. No new dependencies.

---

### Task 1: Rewrite `SectionTransition.tsx` with straight-line git graph
...
Read `src/components/SectionTransition.tsx` to understand the existing DPR/resize/visibility scaffolding you are preserving.

**Step 2: Replace the entire file with the implementation below**

The following is the complete new file. Copy it verbatim.
...
Expected: zero errors. Fix any found before committing.

**Step 4: Commit**

```bash
git add src/components/SectionTransition.tsx
git commit -m "feat(transition): straight-line git graph with sweep animation"
```

Report commit SHA and any issues.
